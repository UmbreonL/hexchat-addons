...
__module_name__ = "Poke day"
__module_version__ = "0.1"
__module_description__ = "SANICK joining nicks to fitting theme: Pokemon"
...
def userJoin(word, word_eol, userdata):
    if hexchat.get_info('network') == config["network"] and hexchat.get_info("channel") == config["channel"]:
        if word[0].lower() in config["override"]:
            ...
            newNick = random.choice(pokemonList)
            ident = word[2].split("@")[0]
            if word[0].title() in pokemonList:
                hexchat.command("sanick %s iLovePokemon%s" % (word[0], random.randint(0,9000)))
            elif "iLovePokemon" in word[0].lower():
                pass
            else:
                try:
                    oldNick = word[0].split('\\')[1]
                except IndexError:
                    oldNick = word[0]
                    hexchat.command("sanick %s %s" % (word[0], newNick.title()))
                else:
                    hexchat.command("sanick %s %s" % (word[0], newNick.title()))
                finally:
                    hexchat.command("sajoin %s #pokemon" % newNick.title())

hexchat.hook_print("Join", userJoin)

def pokenick(word, word_eol, userdata):
    try:
        word[1]
        print(word[1])
    except IndexError:
        ...
        for name in word:
            ...
            hexchat.command("sajoin %s #pokemon" % newNick.title())
hexchat.hook_command("pokenick", pokenick)